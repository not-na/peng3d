...
class ButtonBackground(Background):
    ...
    def init_bg(self):
        ...
        self.vlist = self.submenu.batch2d.add(
            self.n_vertices,
            GL_QUADS,
            pyglet.graphics.OrderedGroup(self.vlist_layer),
            "v2f",
            "c3B",
        )
    ...
    def pressed(self):
        ...
        """
        return self.change_on_press and (
            self.widget.pressed or getattr(self.widget, "focussed", False)
        )
...
class ImageBackground(Background):
    ...
    def __init__(
        self, widget, bg_idle=None, bg_hover=None, bg_disabled=None, bg_pressed=None
    ):
        self.widget = widget
    ...
    def init_bg(self):
        ...
        self.bg_group = pyglet.graphics.TextureGroup(
            _FakeTexture(*self.bg_texinfo),
...
class FramedImageBackground(ImageBackground):
    ...
    def __init__(
        ...
        super(FramedImageBackground, self).__init__(
            widget, bg_idle, bg_hover, bg_disabled, bg_pressed
        )

        if tex_size is None:
            if (not (isinstance(bg_idle, list) or isinstance(bg_idle, tuple))) and len(
                bg_idle
            ) != 2:
                raise TypeError(
                    "Invalid type or length of bg_idle for auto-tex_size, please specify tex_size"
                )
    ...
    def init_bg(self):
        ...
        self.vlist_corners = self.submenu.batch2d.add(
            16, GL_QUADS, self.bg_group, "v2f", "t3f"
        )
        self.vlist_edges = self.submenu.batch2d.add(
            16, GL_QUADS, self.bg_group, "v2f", "t3f"
        )
        self.vlist_center = self.submenu.batch2d.add(
    ...
    def redraw_bg(self):
        ...
        if not self.widget.enabled:
            ...
            ) = self.transform_texture(self.bg_disabled, amx, amy, rx, ry)
        elif self.pressed:
            (
                self.vlist_corners.tex_coords,
                self.vlist_edges.tex_coords,
                self.vlist_center.tex_coords,
            ) = self.transform_texture(self.bg_pressed, amx, amy, rx, ry)
        elif self.widget.is_hovering:
            (
                self.vlist_corners.tex_coords,
                self.vlist_edges.tex_coords,
                self.vlist_center.tex_coords,
            ) = self.transform_texture(self.bg_hover, amx, amy, rx, ry)
        else:
            (
                self.vlist_corners.tex_coords,
                self.vlist_edges.tex_coords,
                self.vlist_center.tex_coords,
            ) = self.transform_texture(self.bg_texinfo, amx, amy, rx, ry)
...
class CheckboxBackground(ButtonBackground):
    ...
    def init_bg(self):
        ...
        self.vlist_check = self.submenu.batch2d.add(
            4,
            GL_QUADS,
            pyglet.graphics.OrderedGroup(10),
            "v2f",
            "c3B",
        )
