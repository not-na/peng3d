...
class WidgetLayer(BasicWidgetLayer):
    ...
    def border(self):
        ...
        if callable(self._border):
            return util.WatchingList(
                self._border(*(self.widget.pos + self.widget.size)),
                self._wlredraw_border,
            )
        else:
    ...
    def offset(self):
        ...
        if callable(self._offset):
            return util.WatchingList(
                self._offset(*(self.widget.pos + self.widget.size)),
                self._wlredraw_offset,
            )
        else:
    ...
    def getPos(self):
        ...
        sx, sy = (
            self.widget.pos[0] + self.border[0] + self.offset[0],
            self.widget.pos[1] + self.border[1] + self.offset[1],
        )
        ex, ey = (
            self.widget.pos[0] + self.widget.size[0] - self.border[0] + self.offset[0],
            self.widget.pos[1] + self.widget.size[1] - self.border[1] + self.offset[1],
        )
        return sx, sy, ex, ey
    ...
    def getSize(self):
        ...
        """
        return (
            self.widget.size[0] - self.border[0] * 2,
            self.widget.size[1] - self.border[1] * 2,
        )
...
class ImageWidgetLayer(WidgetLayer):
    ...
    def initialize(self):
        self.img_vlist = self.widget.submenu.batch2d.add(
            4,
            GL_QUADS,
            self.img_group,
            "v2f",
...
class FramedImageWidgetLayer(DynImageWidgetLayer):
    ...
    def __init__(
        ...
        if tex_size is None:
            ...
            self.tsx, self.tsy = self.widget.peng.resourceMgr.getTexSize(
                *t
            )  # Texture Size in pixels
        else:
            self.tsx, self.tsy = tex_size
        self.frame_x = list(
            map(lambda x: x * (self.tsx / sum(frame[0])), frame[0])
        )  # Frame Size in the texture, in pixels
        ...
        for i in frame:
            if (self.repeat_edge or self.repeat_center) and i[1] == 0:
                raise ValueError(
                    "Cannot repeat edges or center with the middle frame being 0"
                )
    ...
    def scale(self):
        ...
        if (
            scale[0] == 0
        ):  # 0 makes the resulting frames similar to the texture frames but scaled up to the widget size
    ...
    def initialize(self):
        ...
        if (self.frame_x[0] + self.frame_x[2]) * self.scale[0] > self.widget.size[
            0
        ] or (self.frame_y[0] + self.frame_y[2]) * self.scale[1] > self.widget.size[1]:
            raise ValueError(f"Scale {self.scale} is too large for this widget")

        self.bg_group = _DynImageGroup(self, self.group)
        self.vlist_corners = self.widget.submenu.batch2d.add(
            16, GL_QUADS, self.bg_group, "v2f", "t3f"
        )
        self.vlist_edges = self.widget.submenu.batch2d.add(
            16, GL_QUADS, self.bg_group, "v2f", "t3f"
        )
        self.vlist_center = self.widget.submenu.batch2d.add(
            4, GL_QUADS, self.bg_group, "v2f", "t3f"
        )
    ...
    def on_redraw(self):
        ...
        if self.repeat_center or self.repeat_edge:
            amx, amy = int(sfcx / fcx), int(
                sfcy / fcy
            )  # Amount of complete textures in an edge
        ...
        (
            self.vlist_corners.tex_coords,
            self.vlist_edges.tex_coords,
            self.vlist_center.tex_coords,
        ) = t
...
class ImageButtonWidgetLayer(DynImageWidgetLayer):
    ...
    def __init__(
        ...
        super(ImageButtonWidgetLayer, self).__init__(
            name,
            widget,
            z_index,
            border,
        ...
        self.widget.addAction(
            "statechanged", lambda: self.switchImage(self.widget.getState())
        )
...
class BaseBorderWidgetLayer(WidgetLayer):
    ...
    def __init__(
        ...
        super(BaseBorderWidgetLayer, self).__init__(
            name, widget, z_index, base_border, base_offset
        )
        ...
        if style == "material" and self.__class__ == BaseBorderWidgetLayer:
            warnings.warn(
                "Material style may have visual artifacts if used with BaseBorderWidgetLayer, use ButtonBorderWidgetLayer instead",
                stacklevel=2,
            )
        elif style == "gradient" and self.__class__ == BaseBorderWidgetLayer:
            warnings.warn(
                "Gradient style may have visual artifacts if used with BaseBorderWidgetLayer, use ButtonBorderWidgetLayer instead",
                stacklevel=2,
            )
        elif style == "oldshadow" and self.__class__ == BaseBorderWidgetLayer:
            warnings.warn(
                "Oldshadow style may have visual artifacts if used with BaseBorderWidgetLayer, use ButtonBorderWidgetLayer instead",
                stacklevel=2,
            )
    ...
    def getColors(self):
        ...
        bg = (
            self.widget.submenu.bg[:3]
            if isinstance(self.widget.submenu.bg, list)
            or isinstance(self.widget.submenu.bg, tuple)
            else [242, 241, 240]
        )
    ...
    def initialize(self):
        self.batch = (
            self.batch if self.batch is not None else self.widget.submenu.batch2d
        )
    ...
    def on_redraw(self):
        ...
        if len(c) == self.n_vertices * 3:  # one color per vertex
            ...
            raise ValueError(
                "Style produced %s colors, but %s required"
                % (len(c) / 3, self.n_vertices)
            )
    ...
    def stretchColors(self, c):
        ...
        raise NotImplementedError(
            "Color stretching is not supported on %s" % self.__class__.__name__
        )
...
class ButtonBorderWidgetLayer(BaseBorderWidgetLayer):
    ...
    def __init__(
        ...
        super(ButtonBorderWidgetLayer, self).__init__(
            name,
            widget,
            z_index,
            base_border,
